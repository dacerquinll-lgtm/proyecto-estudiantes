...
st.markdown("""
    ...
    div[data-testid="stAlert"] { background-color: #d4edda !important; border: 1px solid #c3e6cb !important; }
    div[data-testid="stAlert"] p, div[data-testid="stAlert"] span { color: #155724 !important; font-weight: bold !important; }
    
    div[data-testid="stTextInput"] label p { color: #0c1c30 !important; font-weight: bold !important; font-size: 1rem !important; }
    div[data-testid="stTextInput"] input { background-color: #ffffff !important; color: #0c1c30 !important; border: 1px solid #ced4da !important; }
    ...
    div.stDownloadButton > button:hover { background-color: #1e7e34 !important; }
    </style>
...
if 'ultimo_diagnostico' in st.session_state:
    ...
    # Cuadro de resultados visible
    st.markdown("### 📋 Resultados del Test")
    with st.container(border=True):
        c1, c2 = st.columns(2)
        c1.metric("Nivel de Estrés", mapa_estres.get(estres))
        c2.metric("Proyección de Rendimiento", mapa_rend.get(rendimiento))
        
        st.write("---")
        st.markdown("**Métricas Analizadas:**")
        cols_m = st.columns(4)
        labels = ["Ansiedad", "Autoestima", "Depresión", "Sueño", "Carga", "Extras", "Apoyo", "Interés"]
        for i in range(8):
            cols_m[i % 4].write(f"• **{labels[i]}:** {datos[i]} / 10")
